...
object Ids:
   ...
   * Id consists of a random base64 value (0-0xff) followed by the Instant.now seconds as base64.
   *
   * @return
   */
  def generateInstanceId(): Id =
    val randomByte = (scala.util.Random.nextInt(256) & 0xFF).toByte
    val nowSeconds = java.time.Instant.now().getEpochSecond
    val bb = ByteBuffer.allocate(5)
    bb.put(randomByte)
    bb.putInt(nowSeconds.toInt)
