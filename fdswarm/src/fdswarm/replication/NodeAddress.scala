 ...
 * This is not suitable to send messages to a node. That's in fdswarm.model.sync.NodeStatus#apiUrl()
 *
 * @param url       this node.
...
case class NodeAddress(url: URL = URI.create("http:///").toURL, instance: Option[Int] = None)
  ...
  override def collectNamedValues(namedValueCollector: replication.NamedValueCollector): Unit =
    {} //todo
