...
/** Id of a collection of QSOs in a calendar hour. Its just a LocalDateTime with
  * only any hour.
  */
case class FdHour private (
    day: Int,
    hour: Int)
    extends Ordered[FdHour]:
  ...
  override def compare(
    ...
    if ret == 0 then this.hour.compareTo(that.hour)
    else ret
